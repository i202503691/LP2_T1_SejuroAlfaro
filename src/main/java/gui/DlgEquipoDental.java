...
public class DlgEquipoDental extends JDialog implements ActionListener {
	...
	void listar() {
		...
		try {
			...
			List<EquipoDental> lstEquiposDentales = manager.createQuery(jpql, EquipoDental.class).getResultList();

			for (EquipoDental equipoDental : lstEquiposDentales) {
				...
				imprimir("Estado......: "+equipoDental.EstadoCompleto());
				imprimir("------------------\n");
	...
	void adicionar() {
		...
		try {
			...
			EquipoDental equipoDental = new EquipoDental(null, nombreCompleto, null, null, estado, null);
	...
	void buscar() {
		...
		try {
			...
			if (equipoDental == null) {
				mensajeAdvertencia("equipo no encontrado");
				return;
